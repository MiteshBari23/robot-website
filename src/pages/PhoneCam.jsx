...
let robotInstance = null; // Prevent multiple robot instances
    ...
    loader.load(
      ...
      (robot) => {
        ...
        socket.on("pose-change", (pose) => {
          ...
          switch (pose) {
            case "legs-forward": set("LLEG_JOINT2", 0.5); set("RLEG_JOINT2", 0.5); break;
            case "legs-back": set("LLEG_JOINT2", -0.5); set("RLEG_JOINT2", -0.5); break;
            case "sit":
              set("LLEG_JOINT2", -0.7); set("LLEG_JOINT3", 1.2);
              set("RLEG_JOINT2", -0.7); set("RLEG_JOINT3", 1.2); break;
            case "stand":
              for (let i = 0; i <= 5; i++) {
                set(`LLEG_JOINT${i}`, 0);
                set(`RLEG_JOINT${i}`, 0);
              } break;
            case "head-rotate": set("HEAD_JOINT0", 0.5); break;
            case "chest-bend": set("CHEST_JOINT1", 0.4); break;
            case "larm-up": set("LARM_JOINT1", -0.6); break;
            case "larm-down": set("LARM_JOINT1", 0.6); break;
            case "rarm-up": set("RARM_JOINT1", -0.6); break;
            case "rarm-down": set("RARM_JOINT1", 0.6); break;
            case "reset":
              Object.keys(robot.joints).forEach((j) => robot.joints[j].setJointValue(0)); break;
            default: break;
          }
        });
      },
